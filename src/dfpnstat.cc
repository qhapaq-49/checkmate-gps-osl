...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "dfl:N:F:s:p:t:vh")) != EOF)
  {
    ...
    {
    case 's':
      dovetailing_seed = static_cast<unsigned int>(atoi(optarg));
      break;
    case 'p':
      dovetailing_prob = static_cast<unsigned int>(atoi(optarg));
      break;
    case 'd':
      debug = true;
      break;
    ...
    case 'N':
      ...
      break;
    case 't':
      table_growth_limit = atoi(optarg);
      break;
#if 0
...
template <class DfpnSearch>
void analyzeCheckmatePV(DfpnSearch &searcher, const NumEffectState &state, Move checkmate_move)
{
  Move ply_move = checkmate_move;
  NumEffectState new_state = state;
  while(1){
    if(ply_move.isInvalid()){
      break;
    }
    std::cerr <<ply_move <<" ";
    new_state.makeMove(ply_move);
    MoveVector moves;
    ...
    ply_move = record.best_move;
  }
  std::cerr<<std::endl<<new_state<<std::endl;
}

template <class DfpnSearch>
std::pair<int, std::vector<Move>> get_depth_reaction(DfpnSearch &searcher, const NumEffectState &state, Move checkmate_move, const int depth){

  if ( checkmate_move.isInvalid()){
  ...
  new_state.generateLegal(moves);
  if(moves.size()==0){
    std::vector<Move> output = {checkmate_move};
    return {depth, output};
  }
  int max_v = -1;
  Move max_move;
  ...
  for (size_t i = 0; i < moves.size(); ++i)
  {
    ...
    HashKey key(tmp);
    const DfpnTable &table = searcher.currentTable();
    DfpnRecordBase record = table.probe(key, PieceStand(WHITE, tmp));
    std::pair<int, std::vector<Move>> res = get_depth_reaction(searcher, tmp, record.best_move, depth + 1);
    int v = res.first;
    if(v > max_v){
      max_v = v;
      best_pv = res.second;
  ...
  std::vector<Move> output = best_pv;
  output.push_back(max_move);
  output.push_back(checkmate_move);
  return {max_v, output};
}

template <class DfpnSearch>
void analyzeCheckmate3(DfpnSearch &searcher, const NumEffectState &state, Move checkmate_move)
{

  std::pair<int, std::vector<Move>> out = get_depth_reaction(searcher, state, checkmate_move, 0);
  std::cout<<"maxdepth"<<out.second.size()<<std::endl;
  for(int i=0;i<out.second.size();++i){
    std::cout<<out.second[out.second.size()-1-i]<<" ";
  }
}
                   ...
                   const std::vector<Move> &moves)
{
  ...
  if ((!force_attack) && !my_king.isPieceStand() && state.inCheck(P))
  {
    // 相手から王手がかかっている
  ...
  time_point timer = clock::now();
  PerfMon clock;
  result = searcher.hasCheckmateMove(state, HashKey(state), path, limit, checkmate_move, Move(), &pv);
  total_cycles += clock.stop();
  real_seconds = elapsedSeconds(timer);
  if (verbose)
    std::cerr << result << "\n";
  ...
  if (result.isCheckmateSuccess())
  {
    ...
    if (debug)
    {
      // analyzeCheckmatePV(searcher, state, checkmate_move);
      analyzeCheckmate3(searcher, state, checkmate_move);
      if (!moves.empty())
...
void search(DfpnSearch &searcher, const char *filename)
{
  ...
  const bool attack = force_attack || state.kingSquare(state.turn()).isPieceStand() || !state.inCheck(state.turn());
